		...
		for (int i : selectedCards){
			for (int k : selectedCards){
				for (int j : selectedCards){
					String cardA = cardAt(i).rank();
					String cardB = cardAt(k).rank();
					String cardC = cardAt(j).rank();
					if (i != k && k != j && i != j){
						if (cardA.equals("jack") && cardB.equals("queen") && cardC.equals("king")){
							return true;
						}
					}
				}
